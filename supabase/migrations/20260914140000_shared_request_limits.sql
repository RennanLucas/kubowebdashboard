CREATE TABLE IF NOT EXISTS kubo_limits_private.request_limits (
  scope text NOT NULL,
...
ALTER TABLE kubo_limits_private.request_limits ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON kubo_limits_private.request_limits FROM PUBLIC, anon, authenticated;
CREATE INDEX IF NOT EXISTS request_limits_expiry ON kubo_limits_private.request_limits(expires_at);
...
BEGIN
  ...
  -- Atomic across connections/workers; a blocked bucket does not grow unbounded.
  ...
  -- Bounded, indexed housekeeping. No IP, JWT or raw identifier is stored.
