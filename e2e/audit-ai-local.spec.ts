...
const quota = { state:null,request_id:null,used:0,remaining:5,limit:5,configured:true,can_generate:true,
  ...
  test(`paid AI panel is readable and only generates on click at ${width}px`,async ({ page },testInfo) => {
    ...
    await page.route("**/*",route => {
      ...
      if (url.pathname.endsWith("/functions/v1/ai-weekly-insights")) {
        ...
        return route.fulfill({ headers,json:generated ? { ...quota,state:"succeeded",used:1,remaining:4,
